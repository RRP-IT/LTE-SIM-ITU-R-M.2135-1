  ...
  if (src->GetNodeType() == NetworkNode::TYPE_ENODEB && dst->GetNodeType () == NetworkNode::TYPE_UE)
    {
	 ...
	 // pathLoss = 128.1 + (37.6 * log10 (distance * 0.001));
	  ...
	  /*
	    ...
	    *  hUE in m
	    ******************************************
	    * dBP- Break point distance, according to
	  ...
	  //std::cout<< " FC1 "<< Fc <<std::endl;

	  if ( distance <= dBP)
	  {

		  pathLoss = 22 *  log10 (distance) + 28 + 20 * log10 (2.6);
	  ...
	  else
	  {
		  ...
		  pathLoss = 40 *  log10 (distance) + 7.8 - 18 * log10 (9.) - 18 * log10 (0.5) + 2 * log10 (2.6);
		  //std::cout<< " FC3 "<< Fc <<std::endl;
	  }
